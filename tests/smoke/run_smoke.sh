...
run_native() {
  ...
  export LLMCTL_ENDPOINT="http://localhost:${STUB_PORT}/v1"

  echo "[smoke] building llmctl …"
  go build -o /tmp/llmctl_smoke "${REPO_ROOT}/cmd/llmctl/"
  ...
  echo "[smoke] running: llmctl review --model devstral-small --no-memory --out ${out_file} ${fixture_repo}"
  /tmp/llmctl_smoke review \
...
run_docker() {
  local os="$1"
  echo "[smoke] building Docker image llmctl-smoke-${os} …"
  docker build -f "${REPO_ROOT}/tests/smoke/Dockerfile.${os}" \
    -t "llmctl-smoke-${os}" \
    "${REPO_ROOT}"
  echo "[smoke] running container llmctl-smoke-${os} …"
  docker run --rm "llmctl-smoke-${os}"
